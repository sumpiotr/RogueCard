...
public class CardGameObject : MonoBehaviour
{
    ...
    private string getDynamicDescription(string data) 
    {
        ...
        {
            desc = desc.Replace("[" + s + "]", "<b>" + GameController.gameController.player.stats.getActualStat(s).ToString() + "</b>");
        }
        return desc;
    }
    ...
    {
        if (isMoving || GameController.gameController.isCardSelected) return;
        FindObjectOfType<CardPreview>().ShowCardDescription(this);
        if (GameController.gameController.player.preparedCard != null) return;
        StopAllCoroutines();
    ...
    private void OnMouseDown()
    {
        ...
        isMoving = true;
        StopAllCoroutines();
        GameController.gameController.isCardSelected = true;
        FindObjectOfType<CardPreview>().HideCardPreview();
    }
    ...
    {
        if (GameController.gameController.player.preparedCard != null) 
        {
            GameController.gameController.player.Undo();
            return; 
        }
        isMoving = false;
        if (transform.localPosition.x < -5 || transform.localPosition.x > -3 + GameController.gameController.player.getHandSize() * 2
            || transform.position.y > transform.parent.transform.position.y + 1) 
        {
            GameController.gameController.player.PlayCard(this);
            GameController.gameController.isCardSelected = false;
            StopAllCoroutines();
            StartCoroutine("UnselectedAnimation");
            return;
        }
